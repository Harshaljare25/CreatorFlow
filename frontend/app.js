...
function navigateToTab(tabId) {
    ...
    switch (tabId) {
        ...
        case 'campaigns':
            ...
            break;
        case 'welcomedm':
            pageTitle.innerText = "Auto Welcome DM Settings";
            pageSubtitle.innerText = "Configure anti-spam message variations to safe-welcome followers.";
            initializeDMConfigForm();
            break;
    }
}
...
/* ==========================================================================
   Welcome DM Setup Logic
   ========================================================================== */
function initializeDMConfigForm() {
    document.getElementById('dm-var-a').value = state.dmConfig.varA;
    document.getElementById('dm-var-b').value = state.dmConfig.varB;
    document.getElementById('dm-var-c').value = state.dmConfig.varC;
    document.getElementById('dm-delay').value = state.dmConfig.delay;

    calculateSafetyMeter();

    // Recompute on variation input
    const textareas = document.querySelectorAll('#dm-config-form textarea, #dm-delay');
    textareas.forEach(el => {
        el.addEventListener('input', calculateSafetyMeter);
    });
}

function calculateSafetyMeter() {
    const varA = document.getElementById('dm-var-a').value.trim();
    const varB = document.getElementById('dm-var-b').value.trim();
    const varC = document.getElementById('dm-var-c').value.trim();
    const delay = document.getElementById('dm-delay').value;

    let variationsCount = 0;
    if (varA) variationsCount++;
    if (varB) variationsCount++;
    if (varC) variationsCount++;
    ...
    if (variationsCount === 1) {
        if (delay === "0") {
            fillPercentage = 15;
            statusText = "DANGER: High risk of ban (1 variant, No Delay)";
            fillColor = "#ef4444";
        } else if (delay === "5") {
            fillPercentage = 40;
            statusText = "MODERATE RISK: Add more message variants";
            fillColor = "#f59e0b";
        } else {
            fillPercentage = 60;
            statusText = "SAFE: Human delay active, but variants recommended";
            fillColor = "#3b82f6";
        }
    } else if (variationsCount === 2) {
        if (delay === "0") {
            fillPercentage = 30;
            statusText = "MODERATE RISK: Set human delay timings";
            fillColor = "#f59e0b";
        } else if (delay === "5") {
            fillPercentage = 75;
            statusText = "SAFE: Good spacing and variation cycle";
            fillColor = "#10b981";
        } else {
            fillPercentage = 85;
            statusText = "VERY SAFE: Recommended setting configuration";
            fillColor = "#10b981";
        }
    } else if (variationsCount === 3) {
        if (delay === "0") {
            fillPercentage = 50;
            statusText = "MODERATE: Messages rotate, but instant DMs trigger bots";
            fillColor = "#f59e0b";
        } else if (delay === "5") {
            fillPercentage = 90;
            statusText = "OPTIMAL SAFETY: Perfect rotation cycle";
            fillColor = "#10b981";
        } else {
            fillPercentage = 100;
            statusText = "MAXIMUM SAFETY: Indistinguishable from human interactions";
            fillColor = "#10b981";
        }
    }

    const fillMeter = document.getElementById('safety-meter-fill');
    const textMeter = document.getElementById('safety-meter-text');

    fillMeter.style.width = fillPercentage + "%";
    fillMeter.style.backgroundColor = fillColor;
    textMeter.innerText = statusText;
}

// Save Welcome DM settings
document.getElementById('dm-config-form').addEventListener('submit', (e) => {
    e.preventDefault();

    state.dmConfig = {
        varA: document.getElementById('dm-var-a').value,
        varB: document.getElementById('dm-var-b').value,
        varC: document.getElementById('dm-var-c').value,
        delay: document.getElementById('dm-delay').value
    };

    saveState();
    alert("DM configurations saved successfully!");
});

// DM Simulator Logic
document.getElementById('sim-follow-btn').addEventListener('click', () => {
    const username = document.getElementById('sim-username').value.trim() || "Aarav_Sharma";
    const messagesBody = document.getElementById('chat-messages-container');
    ...
    messagesBody.innerHTML = `
        <div class="chat-info-banner"><i class="fa-solid fa-bell"></i> System Alert: ${username} started following you!</div>
    `;

    // Stage 1: Incoming message (user followed)
    setTimeout(() => {
        const infBox = document.createElement('div');
        infBox.className = 'chat-info-banner';
        infBox.innerHTML = `<i class="fa-solid fa-circle-notch fa-spin"></i> Auto-automation queue holds message for randomized safety delay...`;
        messagesBody.appendChild(infBox);
        messagesBody.scrollTop = messagesBody.scrollHeight;

        // Stage 2: Triggering random DM variant
        setTimeout(() => {
            // Remove processing indicator
            infBox.remove();

            // Collect active variations from state
            const variants = [];
            if (state.dmConfig.varA) variants.push(state.dmConfig.varA);
            if (state.dmConfig.varB) variants.push(state.dmConfig.varB);
            if (state.dmConfig.varC) variants.push(state.dmConfig.varC);

            if (variants.length === 0) {
                const errorBox = document.createElement('div');
                errorBox.className = 'chat-info-banner';
                errorBox.style.color = 'var(--danger)';
                errorBox.innerText = "Error: No welcome messages saved in configuration!";
                messagesBody.appendChild(errorBox);
                return;
            }

            // Select random variation
            const randomIndex = Math.floor(Math.random() * variants.length);
            let rawText = variants[randomIndex];
            
            // Replace placeholder token {name}
            const processedText = rawText.replace(/{name}/g, username);
            ...
            msgEl.innerHTML = `
                ...
                <span class="chat-msg-time">${currentTimeStr}</span>
            `;
